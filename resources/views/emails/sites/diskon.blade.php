...
| Nama Barang | Kategori | Harga Awal | Diskon | Tanggal Berakhir | Harga Diskon |
| :-------------: |:------------:| :-------------:| :--------:| :------------------:| :---------------: |
@foreach($data as $d)
| {{$d->nama_barang}} | @if($d->kategori == 1) Alat Rumah @elseif($d->kategori == 2) Alat Kebersihan @elseif($d->kategori == 3) Alat Dapur @elseif($d->kategori == 4) Otomotif @elseif($d->kategori == 5) Peralatan Elektronik @elseif($d->kategori == 6) Olahraga & Outdoor @elseif($d->kategori == 7) Lain-lain @endif | Rp. {{number_format($d->harga_jual, 0, ',', '.')}},- | {{$d->diskon}}% | {{Carbon\Carbon::parse($d->tgl_aktif)->translatedFormat('d F Y')}} | Rp. {{$d->harga_diskon}},-
@endforeach
@endcomponent
